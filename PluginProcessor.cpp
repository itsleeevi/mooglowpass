...
{
	filterArray = new Moog[getNumInputChannels()];

	for (int filterNumber = 0; filterNumber < getNumInputChannels(); ++filterNumber)
	{
		filterArray[filterNumber] = Moog();
		filterArray[filterNumber].calc();
		filterArray[filterNumber].init(sampleRate);
	}
}
...
void LowPassFilterAudioProcessor::processBlock(AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
	...
	for (int channel = 0; channel < totalNumInputChannels; ++channel)
	{
		...
		for (int i = 0; i < buffer.getNumSamples(); i++)
		{
			...
			const float output = filterArray[channel].process4pLowPass(inSample);
...
void LowPassFilterAudioProcessor::updateFilterFreq(double freq) 
{
	for (int filterNumber = 0; filterNumber < getNumInputChannels(); ++filterNumber)
	{
		filterArray[filterNumber].updateFreq(freq);
		filterArray[filterNumber].calc();
	}
}

void LowPassFilterAudioProcessor::updateFilterRes(double res) 
{
	for (int filterNumber = 0; filterNumber < getNumInputChannels(); ++filterNumber)
	{
		filterArray[filterNumber].updateRes(res);
		filterArray[filterNumber].calc();
	}
}

/*void LowPassFilterAudioProcessor::setFilterType(int ID)
{
	for (int filterNumber = 0; filterNumber < getNumInputChannels(); ++filterNumber)
	{
		filterArray[filterNumber].updateFilterType(ID);
	} 
} */
/*
void Moog::updateFilterType(int ID)
{
	filterID = ID;
}
*/
